const CONFIG = {
    MINING: "0xa70147A41F10e84D25A97997d7e2507096F86BAD", // ADRESSE CONTRAT MINING V2
    USDT: "0xc2132D05D31c914a87C6611C10748AEb04B58e8F", // Polygon USDT
    FTA: "0x5c418b12c7e9c2A8e9A71A68c6d9b319E7B1d1fd", // ADRESSE TOKEN FTA
    CHAIN_ID: 137,
...
const i18n = {
    en: {
        ...
        machineId: "Machine ID (e.g., 0)", plug: "PLUG IN ⚡", noRigs: "You don't have any machines yet.",
        ...
        sendTitle: "Send", receiveTitle: "Receive Funds", receiveDesc: "Send only POL, USDT or FTA to this address on the Polygon network.",
        tapCopy: "Tap to copy", recipientAddr: "Recipient address (0x...)", amount: "Amount", confirmSend: "CONFIRM SEND",
        sending: "Sending...", sentSuccess: "Sent successfully!", addrCopied: "Address copied!", invalidAddr: "Invalid address", sendFirst: "Send funds to this address"
    },
    fr: {
        ...
        machineId: "ID Machine (ex: 0)", plug: "BRANCHER ⚡", noRigs: "Vous n'avez pas encore de machines.",
        ...
        sendTitle: "Envoyer", receiveTitle: "Recevoir des fonds", receiveDesc: "Envoyez uniquement POL, USDT ou FTA à cette adresse sur le réseau Polygon.",
        tapCopy: "Appuyez pour copier", recipientAddr: "Adresse du destinataire (0x...)", amount: "Montant", confirmSend: "CONFIRMER ENVOI",
    ...
    de: {
        ...
        plugMachine: "🔌 Maschine anschließen", plugDesc: "Geben Sie die ID Ihrer Offline-Maschine ein und wählen Sie eine Batterie.",
        machineId: "Maschinen-ID (z.B. 0)", plug: "ANSCHLIESSEN ⚡", noRigs: "Sie haben noch keine Maschinen.",
        ...
        pluggingIn: "Anschließen...", pluggedIn: "Maschine erfolgreich angeschlossen! ⚡", invalidAmount: "Ungültiger Betrag",
        swapping: "Tauschen...", swapSuccess: "Tausch erfolgreich!", claiming: "Einforderung...", claimed: "Belohnungen eingefordert!",
        error: "Fehler", days: "Tage", rig: "RIG", send: "Senden", receive: "Empfangen",
        sendTitle: "Senden", receiveTitle: "Gelder empfangen", receiveDesc: "Senden Sie nur POL, USDT oder FTA an diese Adresse im Polygon-Netzwerk.",
        tapCopy: "Zum Kopieren tippen", recipientAddr: "Empfängeradresse (0x...)", amount: "Betrag", confirmSend: "SENDUNG BESTÄTIGEN",
        sending: "Senden...", sentSuccess: "Erfolgreich gesendet!", addrCopied: "Adresse kopiert!", invalidAddr: "Ungültige Adresse", sendFirst: "Gelder an diese Adresse senden"
    },
    zh: {
        connect: "连接", refTitle: "👥 推荐系统", refDesc: "输入推荐人地址进行绑定。", bindRef: "绑定",
        power: "算力", ftaSec: "FTA / 秒", pending: "待领取", fta: "FTA",
        miningActive: "挖矿中", noMachine: "无机器", claim: "领取", shopTitle: "⛏️ 商店",
        machines: "矿机", batteries: "电池", buy: "购买", myAssets: "⚙️ 钱包与资产", walletBal: "💰 余额",
        plugMachine: "🔌 插入机器", plugDesc: "输入您离线机器的ID并选择一块电池。",
        machineId: "机器ID (例如: 0)", plug: "插入 ⚡", noRigs: "您还没有机器。",
        ...
        pluggingIn: "插入中...", pluggedIn: "机器插入成功！ ⚡", invalidAmount: "无效金额",
        swapping: "兑换中...", swapSuccess: "兑换成功！", claiming: "领取中...", claimed: "奖励已领取！",
        error: "错误", days: "天", rig: "矿机", send: "发送", receive: "接收",
        sendTitle: "发送", receiveTitle: "接收资金", receiveDesc: "仅向 Polygon 网络上的此地址发送 POL、USDT 或 FTA。",
        tapCopy: "点击复制", recipientAddr: "接收方地址 (0x...)", amount: "金额", confirmSend: "确认发送",
        sending: "发送中...", sentSuccess: "发送成功！", addrCopied: "地址已复制！", invalidAddr: "无效地址", sendFirst: "向此地址发送资金"
    },
    sg: {
        connect: "Connect", refTitle: "👥 Referral System", refDesc: "Enter your referrer's address to link.", bindRef: "BIND",
        power: "POWER", ftaSec: "FTA / sec", pending: "PENDING", fta: "FTA",
...
class Application {
    constructor() {
        ...
        this.ftaDecimals = 8; this.usdtDecimals = 6;
    ...
    applyTranslations() {
        ...
        document.querySelector('#view-my-rigs .card:nth-child(2) h3').innerText = `🔋 ${this.t('myBatteries')}`;
        ...
        document.querySelector('#no-rigs p').innerText = this.t('noRigs');
        ...
        swapHeaders[0].innerText = this.t('youPay');
        swapHeaders[1].innerText = this.t('youReceive');
        document.querySelector('#view-swap .btn-primary').innerText = this.t('swap');

        const navSpans = document.querySelectorAll('.nav-item span');
        navSpans[0].innerText = this.t('home');
        navSpans[1].innerText = this.t('shop');
        navSpans[2].innerText = this.t('assets');
        navSpans[3].innerText = this.t('swapNav');
        
        const daysArr = [7, 15, 30, 90, 180, 270, 365];
        const selectEl = document.getElementById('plug-battery-type');
        for(let i=0; i<selectEl.options.length; i++) { selectEl.options[i].text = `${daysArr[i]} ${this.t('days')}`; }
    }
    ...
    async initContracts() {
        ...
        try { this.ftaDecimals = await this.contracts.fta.decimals(); } catch(e) { this.ftaDecimals = 8; }
    ...
    async updateData() {
        ...
        try {
            ...
            document.getElementById('bal-pol').innerText = pB;
            document.getElementById('bal-usdt').innerText = uB;
            document.getElementById('bal-fta').innerText = fB;
            
            document.getElementById('bal-pol-2').innerText = pB;
            document.getElementById('bal-usdt-2').innerText = uB;
            document.getElementById('bal-fta-2').innerText = fB;
    ...
    async renderShop() {
        ...
        if (this.shopViewMode === 'machines') {
            if (this.shopMachinesData.length === 0) await this.fetchMachines();
            this._renderShopMachinesHTML(container);
        } else {
            if (this.shopBatteriesData.length === 0) await this.fetchBatteries();
    ...
    async fetchMachines() {
        ...
        try {
            ...
            for(let i=0; i<count; i++) {
                ...
                const priceUsdt = parseFloat(ethers.formatUnits(data.price, this.usdtDecimals));
                ...
                this.shopMachinesData.push({ price: priceUsdt, power: power, priceRaw: data.price });
            }
        } catch(e) { console.error("Shop Machines Error", e); }
        this.isLoadingShop = false;
    }
    ...
    async fetchBatteries() {
        this.isLoadingShop = true;
        try {
            const count = await this.contracts.mining.getBatteryCount();
            const promises = [];
            for(let i=0; i<count; i++) promises.push(this.contracts.mining.batteryTypes(i));
            const results = await Promise.all(promises);
            this.shopBatteriesData = [];
            for(let i=0; i<count; i++) {
                const data = results[i];
                const priceUsdt = parseFloat(ethers.formatUnits(data.price, this.usdtDecimals));
                const days = Number(data.duration) / 86400;
                this.shopBatteriesData.push({ price: priceUsdt, days: days, priceRaw: data.price });
            }
        } catch(e) { console.error("Shop Batteries Error", e); }
        this.isLoadingShop = false;
    }

    _renderShopMachinesHTML(container) {
        container.innerHTML = ''; container.style.gridTemplateColumns = '1fr 1fr'; 
        const machineIcons = ["💻", "🖥️", "⛏️", "🏭", "🚀", "🔋"]; 
        for(let i=0; i<this.shopMachinesData.length; i++) {
            const data = this.shopMachinesData[i];
            const div = document.createElement('div'); div.className = 'rig-item';
            div.innerHTML = `<div><span class="shop-icon">${machineIcons[i % machineIcons.length]}</span><span class="rig-name">${this.t('rig')} ${i+1}</span><span class="rig-power">${data.power.toFixed(5)} FTA/s</span></div><div><span class="rig-price">${data.price.toFixed(2)} $</span><button class="btn-primary" style="padding:8px; font-size:0.8rem" onclick="App.buyMachine(${i})">${this.t('buy')} (${this.payMode})</button></div>`;
            container.appendChild(div);
        }
    }

    _renderShopBatteriesHTML(container) {
        container.innerHTML = ''; container.style.gridTemplateColumns = '1fr 1fr 1fr'; 
        const batteryIcons = ["🔋", "⚡", "🔌", "💫", "🌟", "✨", "💎"]; 
        for(let i=0; i<this.shopBatteriesData.length; i++) {
            const data = this.shopBatteriesData[i];
            const div = document.createElement('div'); div.className = 'battery-shop-item';
            div.innerHTML = `<span class="shop-icon" style="font-size: 2rem;">${batteryIcons[i % batteryIcons.length]}</span><div class="battery-name">${data.days} ${this.t('days')}</div><div class="battery-price">${data.price.toFixed(2)} $</div><button class="btn-primary" style="padding:6px; font-size:0.75rem" onclick="App.buyBattery(${i})">${this.t('buy')} (${this.payMode})</button>`;
            container.appendChild(div);
        }
    }
    ...
    async buyMachine(id) {
        ...
        try {
            ...
            if (this.payMode === 'USDT') {
                ...
                if (allow < m.priceRaw) { this.setLoader(true, this.t('approveUsdt')); await (await this.contracts.usdt.approve(CONFIG.MINING, m.priceRaw)).wait(); }
                ...
                const ftaCost = await this.contracts.mining.getFtaCostForUsdtSell(m.priceRaw);
                const ftaCostBigInt = BigInt(ftaCost.toString());
                const ftaAmountToApprove = ftaCostBigInt + (ftaCostBigInt / 10n); 
                const allow = await this.contracts.fta.allowance(this.user, CONFIG.MINING);
                if (allow < ftaAmountToApprove) { this.setLoader(true, this.t('approveFta')); await (await this.contracts.fta.approve(CONFIG.MINING, ftaAmountToApprove)).wait(); }
                this.setLoader(true, this.t('confirming'));
    ...
    async buyBattery(id) {
        ...
        try {
            ...
            if (this.payMode === 'USDT') {
                const allow = await this.contracts.usdt.allowance(this.user, CONFIG.MINING);
                if (allow < b.priceRaw) { this.setLoader(true, this.t('approveUsdt')); await (await this.contracts.usdt.approve(CONFIG.MINING, b.priceRaw)).wait(); }
                this.setLoader(true, this.t('confirming'));
                await (await this.contracts.mining.buyBattery(id)).wait();
            } else {
                this.setLoader(true, this.t('calcFta'));
                const ftaCost = await this.contracts.mining.getFtaCostForUsdtSell(b.priceRaw);
                const ftaCostBigInt = BigInt(ftaCost.toString());
                const ftaAmountToApprove = ftaCostBigInt + (ftaCostBigInt / 10n); 
                const allow = await this.contracts.fta.allowance(this.user, CONFIG.MINING);
                if (allow < ftaAmountToApprove) { this.setLoader(true, this.t('approveFta')); await (await this.contracts.fta.approve(CONFIG.MINING, ftaAmountToApprove)).wait(); }
    ...
    async renderAssets() {
        if(!this.user) return;
        const batteryContainer = document.getElementById('battery-inventory');
        batteryContainer.innerHTML = '';
        try {
            const bCount = await this.contracts.mining.getBatteryCount();
            const daysArr = [7, 15, 30, 90, 180, 270, 365];
            for(let i=0; i<bCount; i++) {
                const div = document.createElement('div'); div.className = 'battery-item';
                div.innerHTML = `<strong>?</strong><span>${daysArr[i]} ${this.t('days')}</span>`;
                batteryContainer.appendChild(div);
            }
        } catch(e) { console.error("Err Batteries", e); }
    }

    async plugInMachine() {
        const mId = document.getElementById('plug-machine-id').value;
        const bType = document.getElementById('plug-battery-type').value;
        if (mId === "" || mId < 0) return this.showToast(this.t('invalidId'), true);
        this.setLoader(true, this.t('pluggingIn'));
        try {
            const tx = await this.contracts.mining.plugInMachine(mId, bType);
            await tx.wait();
    ...
    async calcSwap() {
        ...
        try {
            ...
            if (isUsdtTo) {
                ...
                document.getElementById('swap-to-in').value = parseFloat(ethers.formatUnits(output, this.ftaDecimals)).toFixed(5);
            } else {
                output = await this.contracts.mining.getUsdtOutForFtaSell(amount);
                document.getElementById('swap-to-in').value = parseFloat(ethers.formatUnits(output, this.usdtDecimals)).toFixed(5);
            }
    ...
    openSend(tokenSymbol) {
        ...
        let balId = 'bal-pol-2';
        if(tokenSymbol === 'USDT') balId = 'bal-usdt-2';
        if(tokenSymbol === 'FTA') balId = 'bal-fta-2';
